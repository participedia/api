DROP FUNCTION IF EXISTS get_object_title_list(integer[], text);
...
SELECT array_agg(get_object_title(
   id,
   coalesce((select language from localized_texts where language = language and thingid = id limit 1), origlang)
))
