CREATE MATERIALIZED VIEW search_index_de AS
...
SELECT
   ...
   texts.body,
   texts.description,
   setweight(to_tsvector('german'::regconfig, texts.title), 'A') ||
   setweight(to_tsvector('german'::regconfig, COALESCE(texts.body, '')), 'D') ||
   setweight(to_tsvector('german'::regconfig, COALESCE(texts.description, '')), 'C')
   AS document
 FROM
   cases
...
SELECT
   ...
   texts.description,
   setweight(to_tsvector('german'::regconfig, texts.title), 'A') ||
   setweight(to_tsvector('german'::regconfig, texts.body), 'D') ||
   setweight(to_tsvector('german'::regconfig, texts.description), 'C')
   AS document
 FROM
   organizations
...
CREATE INDEX idx_fts_search_de ON search_index_de USING gin(document);
