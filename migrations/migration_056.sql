CREATE MATERIALIZED VIEW search_index_it AS
...
SELECT
   ...
   texts.body,
   texts.description,
   setweight(to_tsvector('italian'::regconfig, texts.title), 'A') ||
   setweight(to_tsvector('italian'::regconfig, COALESCE(texts.body, '')), 'D') ||
   setweight(to_tsvector('italian'::regconfig, COALESCE(texts.description, '')), 'C')
   AS document
 FROM
   cases
...
SELECT
   ...
   texts.description,
   setweight(to_tsvector('italian'::regconfig, texts.title), 'A') ||
   setweight(to_tsvector('italian'::regconfig, texts.body), 'D') ||
   setweight(to_tsvector('italian'::regconfig, texts.description), 'C')
   AS document
 FROM
   organizations
...
CREATE INDEX idx_fts_search_it ON search_index_it USING gin(document);
