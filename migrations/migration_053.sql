CREATE MATERIALIZED VIEW search_index_fr AS
...
SELECT
   ...
   texts.body,
   texts.description,
   setweight(to_tsvector('french'::regconfig, texts.title), 'A') ||
   setweight(to_tsvector('french'::regconfig, COALESCE(texts.body, '')), 'D') ||
   setweight(to_tsvector('french'::regconfig, COALESCE(texts.description, '')), 'C')
   AS document
 FROM
   cases
...
SELECT
   ...
   texts.description,
   setweight(to_tsvector('french'::regconfig, texts.title), 'A') ||
   setweight(to_tsvector('french'::regconfig, texts.body), 'D') ||
   setweight(to_tsvector('french'::regconfig, texts.description), 'C')
   AS document
 FROM
   organizations
...
CREATE INDEX idx_fts_search_fr ON search_index_fr USING gin(document);
