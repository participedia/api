CREATE MATERIALIZED VIEW search_index_nl AS
...
SELECT
   ...
   texts.body,
   texts.description,
   setweight(to_tsvector('dutch'::regconfig, texts.title), 'A') ||
   setweight(to_tsvector('dutch'::regconfig, COALESCE(texts.body, '')), 'D') ||
   setweight(to_tsvector('dutch'::regconfig, COALESCE(texts.description, '')), 'C')
   AS document
 FROM
   cases
...
SELECT
   ...
   texts.description,
   setweight(to_tsvector('dutch'::regconfig, texts.title), 'A') ||
   setweight(to_tsvector('dutch'::regconfig, texts.body), 'D') ||
   setweight(to_tsvector('dutch'::regconfig, texts.description), 'C')
   AS document
 FROM
   organizations
...
CREATE INDEX idx_fts_search_nl ON search_index_nl USING gin(document);
